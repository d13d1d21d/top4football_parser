...
class Parser:
    ...
    def get_n_pages(self) -> int:
        return self.n_pages(
            PER_PAGE,
            int(BeautifulSoup(
                self.proxy_client.http("GET", self.BASE + self.path + f"?priceFrom=10&priceTo=300").text,
                "html.parser"
    ...
    def get_variations(self, url: str) -> list[Product]:
        v = []
        try:
            page = BeautifulSoup(self.proxy_client.http("GET", url).text, "html.parser")
            if data := json.loads(
                ...
                if COLORS.get(
                    ...
                    color = dom_color_name(
                        *ColorThief(self.proxy_client.http("GET", images[0], stream=True).raw).get_color(10)
                    )
